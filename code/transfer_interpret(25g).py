...
def split_dataset_transfer(data):
    if 'X_CC_25.npy' not in os.listdir('dataset_save'):
        all_eluent_ratio = []
        all_descriptor=np.load('dataset_save/all_descriptor_CC_25.npy')
        dataset = []
        y=[]
        for i in tqdm(range(len(data['smiles']))):
            if data['t1'][i]*data['speed'][i]>120:
                continue
            if data['t2'][i] * data['speed'][i]>240:
                continue
            smile = data['smiles'][i]
        ...
        X=np.array(dataset)
        y=np.array(y)
        np.save('dataset_save/X_CC_25.npy', X)
        np.save('dataset_save/Y_CC_25.npy', y)
        np.save('dataset_save/eluent_ratio_25.npy', all_eluent_ratio)
    else:
        X=np.load('dataset_save/X_CC_25.npy')
        y=np.load('dataset_save/Y_CC_25.npy')
    np.save('dataset_save/X_max_CC_25.npy',np.max(X,axis=0))
    np.save('dataset_save/X_min_CC_25.npy', np.min(X, axis=0))
...
X_CC = np.load('dataset_save/X_CC_25.npy')
y_CC= np.load('dataset_save/Y_CC_25.npy')
...
X_CC_max=np.load('dataset_save/X_max_CC_25.npy')
X_CC_min=np.load('dataset_save/X_min_CC_25.npy')
...
def test_all_eq_origin(X_CC,y_CC):
    ...
    save_result.to_csv(f"result_save/tree_adopt/result_25_origin.csv")
    with open(f"result_save/tree_adopt/R_2_25_origin.log", "w") as f:
...
def save_Rf_t(X_CC,y_CC):
    def model(x, a, b, c):
        return a * x[0] / (b * x[1] + c)

    from scipy.optimize import curve_fit
    result= pd.read_csv('result_save/tree_adopt/result_25_origin.csv')
